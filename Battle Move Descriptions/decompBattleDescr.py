...
for i in range(1, 799):
    ...
    previousFile.close()
    size = os.stat("battleDescrFiles/" + text.split("\n")[i - 1] + "_" + str(i - 1) + ".bin").st_size
...
binn.write(var.to_bytes(4, "little"))
for i in range(799):
    binn.write(open("battleDescrFiles/" + text.split("\n")[i] + "_" + str(i) + ".bin", "rb").read())            
binn.close()

binn = open("combined.bin", "rb").read()
open("_Decompressed_00000028.bin", "wb").write(binn[0:8192])
open("_Decompressed_00000B98.bin", "wb").write(binn[8192:16384])
open("_Decompressed_000010F0.bin", "wb").write(binn[16384:os.stat("combined.bin").st_size])
